...
import log
from main import OutputRepresentation
...
class Communication():
    ...
    def serial_read(self, output_representation):
        try:
            if self.is_connected:
                data = self.serial_port.read_all()

                if len(data) > 0:
                    if output_representation == OutputRepresentation.STRING:
                        data_str = data.decode("utf-8")
                        log.log_data(self.gui, f"{RX_TAG}{data_str}")

                    elif output_representation == OutputRepresentation.HEX_LIST:
                        data_hex = ', '.join(hex(d) for d in data)
                        log.log_data(self.gui, f"{RX_TAG}[{data_hex}]")

                    elif output_representation == OutputRepresentation.ASCII_LIST:
                        data_ascii = []
                        for d in data:
                            if d < 128:
                        ...
                        log.log_data(self.gui, f"{RX_TAG}[{data_ascii}]")
                    else:
                        log.log_error(self.gui, "Unknown output representation format: %s" % str(output_representation))
        except Exception as e:
